...
def run_demo(model, infrared_path, visible_path, output_path_root, f_type):
    vi = cv2.imread(visible_path)
    ...
    for img_fusion in img_fusion_list:
        file_name = os.path.split(visible_path)[-1]
        output_path = os.path.join(output_path_root, file_name)
        output_count += 1
        # save images
        utils.save_image_test(img_fusion, output_path, vi)
        print(output_path)
...
def main():
    ...
    with torch.no_grad():
        ...
        for img in os.listdir(os.path.join(test_path, 'vi')):
            infrared_path = os.path.join(test_path,'ir',img)
            visible_path = os.path.join(test_path,'vi',img)
            run_demo(model, infrared_path, visible_path, output_path, f_type)
        # end = time.time()
