...
import cv2  # 替换 scipy.misc 为 OpenCV
...
def save_image_test(img_fusion, output_path, vi):
    ...
    if len(vi.shape) == 3:
        image_ycbcr = cv2.cvtColor(vi, cv2.COLOR_BGR2YCrCb)
        Cb = image_ycbcr[:,:,1]
        Cr = image_ycbcr[:,:,2]
        img_fusion = np.stack([img_fusion, Cb, Cr], axis=2)
        img_fusion = cv2.cvtColor(img_fusion, cv2.COLOR_YCrCb2BGR)
    print(img_fusion.shape)
    cv2.imwrite(output_path, img_fusion)  # 使用 OpenCV 保存图像
...
def get_image(path, height=224, width=224, flag=False):
    ...
    # 判断是否是三通道图像
    if len(image.shape) == 3 and image.shape[2] == 3:  # 三通道图像
        # 将 BGR 图像转换为 YCbCr
        image_ycbcr = cv2.cvtColor(image, cv2.COLOR_BGR2YCrCb)
        # 只取 Y 通道（灰度信息）
        image = image_ycbcr[:, :, 0]
    elif len(image.shape) == 2:  # 单通道图像（灰度图）
        image = image
    else:
        raise ValueError("Unsupported image format: must be either grayscale or 3-channel BGR.")
...
def get_test_images(paths, height=None, width=None, flag=False):
    ...
    for path in paths:
        image = get_image(path, height, width, flag)
        if height is not None and width is not None:
            image = cv2.resize(image, (width, height), interpolation=cv2.INTER_NEAREST)  # 使用 OpenCV 调整图像大小
        base_size = 224
        h = image.shape[0]
        w = image.shape[1]
